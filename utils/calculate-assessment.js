...
module.exports = {
  ...
  GetAttainmentOfEachPerformanceIndicator: async (id_assessment) => {
    try {
      const GradingCategory = await FetchGradingCategory();
      const Assessment = await FetchAssessmentById(id_assessment);
      ...
      for (const item of data) {
        ...
        for (const Step5 of PERCENTAGE_WITHIN_EACH_CATEGORY) {
          ...
          let mid_exam = 0;
          if (item.mid_exam) mid_exam = Step5.mid_exam;

          let final_exam = 0;
          if (item.final_exam) final_exam = Step5.final_exam;
        ...
        for (const Step7 of PERCENTAGE_WITHIN_EACH_PROFICIENCY_LEVEL) {
          ...
          let mid_exam = 0;
          if (item.mid_exam) mid_exam = Step7.mid_exam;

          let final_exam = 0;
          if (item.final_exam) final_exam = Step7.final_exam;
